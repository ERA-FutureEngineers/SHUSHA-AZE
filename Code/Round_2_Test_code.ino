...
int targetAngle = 0;  // Target turn angle in degrees
...
void setup() {
  ...
  // Gyro calibration
  offsetZ = gyro.getRotationZ() / 131.0;  // Initial offset for Z-axis
...
void loop() {
  ...
  while (L == 1 || R == 1) {
    ...
    if (redDetected) {
      ...
      if (currentMillis - previousMillis >= 3000 && leftdist >= 100 and rightdist < 100) { 
...
const float alpha = 0.98;  // Weight for complementary filter, adjust if needed

void updateGyroAngle() {
  int16_t gz, ax, ay;  // Variables for gyro Z and accelerometer X and Y readings
  ...
  // Calculate angle from gyro data and update currentAngle
  ...
  accelAngle = atan2(ay, ax) * (180.0 / PI);  // Convert to degrees
  
  // Apply complementary filter to combine gyro and accelerometer angles
  filteredAngle = alpha * (filteredAngle + rotationZ * deltaTime) + (1 - alpha) * accelAngle;

  if(filteredAngle >= 0.25 and filteredAngle <= -0.25)
